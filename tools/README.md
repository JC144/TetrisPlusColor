...
- **Windows** — RGBDS binaries are the win64 build (everything else is portable Python)
...
then the safety checks below). `build.bat` is a thin wrapper around it.
`--skip-gen` skips the generators, `--verify` chains `verify_screens.py`.
...
| [`build.py`](build.py) | The unified build: generators → rgbasm/rgblink/rgbfix → `check_rom.py` → `list_patches.py --check`. [`build.bat`](build.bat) is a 2-line wrapper around it. |
